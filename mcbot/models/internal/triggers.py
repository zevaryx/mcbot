...
class BaseTrigger(ABC):
    ...
    def __or__(self, other: "BaseTrigger") -> "OrTrigger":
        return OrTrigger(self, other) # type: ignore
...
class IntervalTrigger(BaseTrigger):
    ...
    def __init__(self, seconds: _t = 0, minutes: _t = 0, hours: _t = 0, days: _t = 0, weeks: _t = 0) -> None:
        self.delta = timedelta(seconds=seconds, minutes=minutes, hours=hours, days=days, weeks=weeks)
        
        if (datetime.now() + self.delta) < datetime.now():
            raise ValueError("Interval values must be in the future!")
...
class CronTrigger(BaseTrigger):
    ...
    def __init__(self, cron: str, tz: tzinfo = timezone.utc) -> None:
        self.cron = cron
        self.tz = tz
...
class OrTrigger(BaseTrigger):
    ...
    def __init__(self, *trigger: BaseTrigger) -> None:
        ...
        self.current_trigger: BaseTrigger = None # type: ignore
